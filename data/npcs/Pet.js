	...
	for(var r in riders as rider) { //Loop the riders on npc
		...
		if(rider.getType() == 1) { //If the rider is an player
			...
			if(ms != 0) {
				if(ms < 0) {
					//A
					var shot = e.npc.shootItem(e.npc.getX()-lengthdir_x(3, rot+90), e.npc.getY(), e.npc.getZ()+lengthdir_z(3, rot+90), e.npc.world.createItem("variedcommodities:spell_fire", 0, 1), 100);
