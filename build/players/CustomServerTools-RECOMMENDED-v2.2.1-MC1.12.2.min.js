...
var onFn = cdata.self.onFns['list'][o];
				...
				var excludeRgx = /\*([\w]+)/;
				var newMatches = [];
				for(var a in args.matches) {
var match = args.matches[a];
...
var id = ids[i];
					...
					var filterArgs = "";
					for(var ex in excludes) {
var excl = excludes[ex];
						filterArgs += "*"+excl+" ";
					}
					...
					var showResultBtns = "";
					for(var i in showResultVals) {
var rv = showResultVals[i];
...
var sbt = sbteams[s];
	...
	registerXCommands([
		['!debug [...matches]', function(pl, args, data){
			...
			var params = getArgParams(args.matches);
			tellPlayer(pl, genDataList(items, args.matches, params.show||10, params.page||1));
...
function genDataList(items, matches, showLen, curPage, navCmd, sortFn) {
...
var match = matches[a];
    ...
    for(var i in items) {
var item = items[i];
        var isExcluded = (arrayOccurs(item, excludes, false, false) > 0);
        if(matches.length == 0 || arrayOccurs(item, matches, false, false) > 0) {
            if(!isExcluded) {
    ...
    //Sort items
    items.sort(sortFn||function(a,b){
    ...
    var maxPages = Math.ceil(curShow/showLen);
    output += "&6&lPage: &5&l"+curPage+"/"+maxPages+"\n";
...
function registerCSTEnchant(id, maxlvl, name, displayName, func, showLvl) {
...
var SCRIPT_VERSION = "CustomServerTools-RECOMMENDED-v2.2.1-MC1.12.2.min.js";
